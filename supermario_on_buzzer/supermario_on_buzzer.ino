// Sources: https://learn.digilentinc.com/Documents/392
// Modifed by Yifan Yu & Uyen Truong, 2021
...
void setup() {
    ...
    double newNoteLength[] = {
        0.0167,
        0.3333
    };

    const char *fullMelody[] = {
...
void playMelody(const char **song, double typicalNoteLength, int tempo, double newNoteLength[], int numLines) {
...
void playLine(const char *songLine, double typicalNoteLength, int tempo, double newNoteLength[]) {
    // remove spaces from song
    char lineNotes[256];
    removeWhiteSpace(songLine, lineNotes);
    int lineNotesLength = strlen(lineNotes);
    for (int i = 0; i < lineNotesLength; i += 2) {
